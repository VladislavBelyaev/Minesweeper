...
import Minesweeper.ui.gui.components.CustomButton;
import Minesweeper.ui.gui.components.CustomDialog;
import Minesweeper.ui.gui.components.CustomPanel;
...
import Minesweeper.time.Timer;
import static Minesweeper.ui.gui.components.UIDesigner.DEFAULT_BACKGROUND;
import static Minesweeper.util.TextUtils.getTimeString;
...
public class GUIGame extends JPanel implements GameViewController
{
    ...
    {
        timer = new Minesweeper.time.Timer();
        score = new ScoresFrame();
        UIDesigner.init();
        if(parent == null)
        {
    ...
    public boolean restartGame()
    {
        ...
        if(!timer.isRunning() && pausePanel != null)
        {
    ...
    public void noticeWinGame()
    {
        if(Minesweeper.isMainController(this))
        {
            JOptionPane.showMessageDialog(parent, "<html><h2>You win! с:</h2><i>Your score " +
                    getTimeString(timer.getSeconds() / 3600) + ":" +
                    getTimeString((timer.getSeconds() % 3600) / 60) + ":" +
                    getTimeString(timer.getSeconds() % 60) + "!</i>");
            ...
            Pair p = new Pair(name + ";" + getTimeString(timer.getSeconds() / 3600) + ":" +
                    getTimeString((timer.getSeconds() % 3600) / 60) + ":" +
                    getTimeString(timer.getSeconds() % 60));
            ScoreRecords.saveRecord(p);
            score.showRecords(parent);
        }
    }
    ...
    {
        timerCountL.setText(getTimeString(timer.getSeconds() / 3600) + ":" + getTimeString((timer.getSeconds() % 3600) / 60) + ":" + getTimeString(timer.getSeconds() % 60));
    }
    ...
    {
        if(timer.isRunning() || isButtonStop)
            return;
